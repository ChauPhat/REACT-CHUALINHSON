...
const QuyGD = () => {
    ...
    const fetchFundData = async () => {
        try {
            const response = await apiClient.get(`/api/quy-doan/get-lich-su-quy-doan?quyDoanId=5`);
    ...
    const handleDownloadExcel = async () => {
        ...
        try {
            const response = await apiClient.post('/api/export-excel/quy', data, {
                params: {
                    filename: 'quy-doan-nganh-thanh.xlsx',
                },
            ...
            link.href = url;
            link.setAttribute('download', 'quy-doan-nganh-thanh.xlsx'); // Tên file
