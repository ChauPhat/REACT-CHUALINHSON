...
const DSNganhThanh = () => {
  ...
  const layDuLieu = async () => {
    try {
      const response = await apiClient.get(`/api/doan-sinh-details?doanId=2`);
      const fetchedData = await Promise.all(response.data.data.map(async (item) => {
        // Lọc các doanSinhDetails không thỏa mãn điều kiện
        const activeDoanSinhDetails = item.doanSinhDetails.filter(detail => detail.isActive && detail.doanId === 2);
